...
#ifndef DEF__MAINTENANCEMISSION_H
#define DEF__MAINTENANCEMISSION_H

#ifdef PRAGMA
#pragma once
#ifndef _MSC_VER
#pragma interface "MaintenanceMission.h"
#endif
#endif
...
// line 732 "model.ump"
// line 1287 "model.ump"
class MaintenanceMission : public InternalMission{
	
	private:
		
		//MaintenanceMission Attributes
		int repairComplexity;
	...
	public:
		...
		//CONSTRUCTOR
		//------------------------
		MaintenanceMission(const string aDescription, const int aDifficulty, const int aDuration, Shelter* aShelter, const string aPriority, Facility* aTargetFacility, const int aRepairComplexity);
		MaintenanceMission(MaintenanceMission& other);
		
		//------------------------
		//STREAM HELPER GROUPDECLARATION
		//------------------------
		friend ostream& operator<<(ostream& os, const MaintenanceMission& dt);
		
		//------------------------
		//PREDEFINED OPERATORS
		//------------------------
		friend bool operator == (MaintenanceMission& Right, MaintenanceMission& Left);
		friend bool operator != (MaintenanceMission& Right, MaintenanceMission& Left){
		  return !( Right == Left);  
		}
		bool operator == (const MaintenanceMission& Right) const{
		  return this == &Right;  
		}
		bool operator != (const MaintenanceMission& Right) const{
		  return this != &Right;  
		}
		MaintenanceMission& operator=(MaintenanceMission& other);
		
		void internalCopy(MaintenanceMission& other);
		bool setRepairComplexity(const int aNewRepairComplexity);
		int getRepairComplexity(void);
		virtual size_t hashCode(void);
		
		//------------------------
		//DESTRUCTOR
		//------------------------
		virtual ~MaintenanceMission();
...
namespace std{
    template<> struct hash<MaintenanceMission *> {
        size_t operator()(MaintenanceMission* ptr ) const {
            return ptr->hashCode();
        }
    };
}
