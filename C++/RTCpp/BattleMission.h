...
#ifndef DEF__BATTLEMISSION_H
#define DEF__BATTLEMISSION_H

#ifdef PRAGMA
#pragma once
#ifndef _MSC_VER
#pragma interface "BattleMission.h"
#endif
#endif
...
// line 863 "model.ump"
// line 1312 "model.ump"
class BattleMission : public ExternalMission{
	
	private:
		
		//BattleMission Attributes
		int enemyForce;
	...
	public:
		...
		//CONSTRUCTOR
		//------------------------
		BattleMission(const string aDescription, const int aDifficulty, const int aDuration, Shelter* aShelter, const string aDestination, const double aTravelDistance, const int aEnemyForce);
		BattleMission(BattleMission& other);
		
		//------------------------
		//STREAM HELPER GROUPDECLARATION
		//------------------------
		friend ostream& operator<<(ostream& os, const BattleMission& dt);
		
		//------------------------
		//PREDEFINED OPERATORS
		//------------------------
		friend bool operator == (BattleMission& Right, BattleMission& Left);
		friend bool operator != (BattleMission& Right, BattleMission& Left){
		  return !( Right == Left);  
		}
		bool operator == (const BattleMission& Right) const{
		  return this == &Right;  
		}
		bool operator != (const BattleMission& Right) const{
		  return this != &Right;  
		}
		BattleMission& operator=(BattleMission& other);
		
		void internalCopy(BattleMission& other);
		bool setEnemyForce(const int aNewEnemyForce);
		int getEnemyForce(void);
		virtual size_t hashCode(void);
		
		//------------------------
		//DESTRUCTOR
		//------------------------
		virtual ~BattleMission();
...
namespace std{
    template<> struct hash<BattleMission *> {
        size_t operator()(BattleMission* ptr ) const {
            return ptr->hashCode();
        }
    };
}
