...
#ifndef DEF__TRADEMISSION_H
#define DEF__TRADEMISSION_H

#ifdef PRAGMA
#pragma once
#ifndef _MSC_VER
#pragma interface "TradeMission.h"
#endif
#endif
...
// line 810 "model.ump"
// line 1302 "model.ump"
class TradeMission : public ExternalMission{
	
	private:
		
		//TradeMission Attributes
	...
	public:
		...
		//CONSTRUCTOR
		//------------------------
		TradeMission(const string aDescription, const int aDifficulty, const int aDuration, Shelter* aShelter, const string aDestination, const double aTravelDistance, const string aTradingPartner);
		TradeMission(TradeMission& other);
		
		//------------------------
		//STREAM HELPER GROUPDECLARATION
		//------------------------
		friend ostream& operator<<(ostream& os, const TradeMission& dt);
		
		//------------------------
		//PREDEFINED OPERATORS
		//------------------------
		friend bool operator == (TradeMission& Right, TradeMission& Left);
		friend bool operator != (TradeMission& Right, TradeMission& Left){
		  return !( Right == Left);  
		}
		bool operator == (const TradeMission& Right) const{
		  return this == &Right;  
		}
		bool operator != (const TradeMission& Right) const{
		  return this != &Right;  
		}
		TradeMission& operator=(TradeMission& other);
		
		void internalCopy(TradeMission& other);
		bool setTradingPartner(const string aNewTradingPartner);
		string getTradingPartner(void);
		virtual size_t hashCode(void);
		
		//------------------------
		//DESTRUCTOR
		//------------------------
		virtual ~TradeMission();
...
namespace std{
    template<> struct hash<TradeMission *> {
        size_t operator()(TradeMission* ptr ) const {
            return ptr->hashCode();
        }
    };
}
